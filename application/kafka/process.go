...
func(k *KafkaProcessor) Consume(){
	...
	for{
		...
		if err == nil{
			k.proccessMessage(msg)
		}
	}
}

func (k *KafkaProcessor) proccessMessage(msg *ckafka.Message){
	transactionsTopic := "transactions"
	transactionConfirmationTopic := "transaction_confirmation"

	switch topic := *msg.TopicPartition.Topic; topic {
	case transactionsTopic:
		k.proccessTransaction(msg)
	case transactionConfirmationTopic:
		k.proccessTransactionConfirmation(msg)
	default:
...
func (k *KafkaProcessor) proccessTransaction(msg *ckafka.Message)error{
	...
	if err != nil{
		fmt.Println("Error resgistering transaction", err)
		return err
	}
...
func (k *KafkaProcessor) proccessTransactionConfirmation(msg *ckafka.Message) error{
	transaction := appmodel.NewTransaction()
	err := transaction.ParseJson(msg.Value)
	if err != nil{
		return err
